...
from src.Utilities.Padding import pkcs7_pad, pkcs7_unpad
from src.Utilities.Byte_Calculations import xor_bytes


def aes_ecb_encrypt(plaintext: bytes, key: bytes, add_padding=True):
    if add_padding:
...
def aes_ecb_decrypt(ciphertext: bytes, key: bytes, remove_padding=False):
    cipher_obj = AES.new(key, AES.MODE_ECB)
...
def aes_cbc_encrypt(plaintext: bytes, key: bytes, nonce: bytes = bytes(AES.block_size), add_padding=True):
    # pad if needed
    if add_padding:
        plaintext = pkcs7_pad(plaintext, block_size=AES.block_size)
...
def aes_cbc_decrypt(ciphertext: bytes, key: bytes, nonce: bytes = bytes(AES.block_size), remove_padding=False):
    # verify input
    if len(nonce) != AES.block_size:
        raise ValueError(f"Nonce must be of size {AES.block_size}")
